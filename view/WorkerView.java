...
public class WorkerView {
	...
	private void displayAdjustedSalaryWorkers() {
	    ...
	    for (WorkerModel worker : workers) {
	        if (worker instanceof History) {
	            History history = (History) worker;
	            System.out.printf("%-5s %-11s %3d  %8.2f  %-6s  %s%n", worker.getId(), worker.getName(), worker.getAge(),
	                    worker.getSalary(), history.getStatus(),
	                    history.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
	        }
	    }
	}
}